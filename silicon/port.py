...
class Junction(JunctionBase):
    ...
    def set_parent_module(self, parent_module: 'Module') -> None:
        ...
        for member_junction in self.get_member_junctions().values():
    ...
    def set_source(self, source: 'Junction') -> None:
        ...
        if self.is_composite():
            ...
            for member_name, member_junction in self.get_member_junctions().items():
                member_junction.set_source(source.get_member_junctions()[member_name])
        else:
            del_source()
            self.source = source
    ...
    def generate_interface(self, back_end: 'BackEnd', port_name: str) -> Sequence[str]:
        ...
        if not self.is_composite():
            ...
            ret_val = []
            for member_name, member_junction in self._member_junctions.items():
    ...
    def __getattr__(self, name: str) -> Any:
        if "_member_junctions" in dir(self) and name in self._member_junctions:
            return self._member_junctions[name]
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if "_member_junctions" in dir(self) and name in self._member_junctions:
            if value is self._member_junctions[name]:
                return
            self._member_junctions[name] <<= value
    ...
    def set_context(self, context: str) -> None:
        ...
        if context is None:
            ...
            assert False
        for member_junction in self._member_junctions.values():
    ...
    def create_member_junction(self, name: str, net_type: 'NetType', reversed: bool) -> None:
        ...
        self._member_junctions[name] = member
    ...
    def get_member_junctions(self) -> Dict[str, 'Junction']:
    ...
    def get_all_member_junctions(self, add_self: bool) -> Sequence['Junction']:
        ...
        ret_val = OrderedSet()
        if self.is_composite():
            for member in self.get_member_junctions().values():
